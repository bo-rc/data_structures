...
Allocator::Allocator(const std::string& studentFile, const std::string& roomFile) : alpha{nullptr}, rooms{nullptr}, roomCount{0}, studentCount{0}, totalCapacity{0}
{
    createLetterGroups();
...
Allocator::Allocator(const Allocator& other)
    : alpha{nullptr}, rooms{nullptr}, roomCount{other.roomCount}, studentCount{other.studentCount}, totalCapacity{other.totalCapacity}
{
    createLetterGroups();
...
Allocator::Allocator() : alpha{nullptr}, rooms{nullptr}, roomCount{0}, studentCount{0}, totalCapacity{0}
{
    /* empty */
}
...
void Allocator::loadRooms(const std::string& file)
{
    ...
    while (list.has_next())
    {
        // Bo: delete i++, put it in rooms' brackets
        rooms[i] = list.next();
