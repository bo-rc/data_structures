...
class kd_tree
{
  public:
    ...
    // helper for find_nearest_neighbor
    void find_nearest_neighbor_kdtree(const point<Dim>& query, point<Dim>& best,
  ...
  private:
     ...
     */
    // helper for kd_tree
    void build_tree(typename std::vector<point<Dim>>::iterator it_points_begin,
                    typename std::vector<point<Dim>>::iterator it_points_end,
                    size_t sorting_dim);

    // quick_select: subroutine for kd_tree

    auto quick_select(typename std::vector<point<Dim>>::iterator it_begin,
                        typename std::vector<point<Dim>>::iterator it_end,
                        typename std::vector<point<Dim> >::iterator::difference_type kth, size_t sorting_dim)
    -> typename std::vector<point<Dim>>::iterator;

    auto partition(typename std::vector<point<Dim>>::iterator it_begin,
                   typename std::vector<point<Dim>>::iterator it_end,
    ...
    size_t distance(const point<Dim>& target, const point<Dim>& current) const;
