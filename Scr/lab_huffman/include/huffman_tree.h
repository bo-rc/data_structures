  ...
  private:
    ...
    class node
    {
      public:
        ...
        node(const node& other) : freq{other.freq}
        {
            const node* other_ptr = &other;
            copy_node(this, other_ptr);
        }

        void copy_node(node* this_ptr, const node* other_ptr)
        {
            if (other_ptr == nullptr)
                return;

            this_ptr->freq = other_ptr->freq;
            copy_node(this_ptr->left.get(), other_ptr->left.get());
            copy_node(this_ptr->right.get(), other_ptr->right.get());
        }
    };
    ...
    // recursive helper for read_tree
    void read(std::unique_ptr<node>& current, binary_file_reader& bfile);
    ...
    void print_queue(std::queue<std::unique_ptr<node>>& q, std::string s);
